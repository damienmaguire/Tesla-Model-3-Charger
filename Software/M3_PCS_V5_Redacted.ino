/*
Redacted version of my very hacky Tesla Model 3 Power Conversion System Controller Software. Alpha version.
New V2 controller will operate with much nicer openinverter based web interface firmware!

V5 features auto enable and disable of PCS and DCDC function when car reports on.
Also features a basic CAN interface to the Zombieverter VCU.
...
Charge function tested on US version to 6.5kw AC.
...
if(vcuMode==0x00) //only enable charge mode if the car is off
{
if((Proximity==Connected)&&(vcuEn==true))  //if we have a evse plug detected and are allowed to enable by the vcu ...
{
...
{
if((Proximity==Unconnected)||(vcuEn==false))  //if plug removed or we are disabled by the vcu
{
    ...
    digitalWrite(EVSE_ACT,LOW); //turn off evse power
